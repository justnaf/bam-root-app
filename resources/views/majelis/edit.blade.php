<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{route('majelis.index')}}">Kajian Management</a> > <span>Edit</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-4">
                <div class="p-6 text-gray-900">
                    <h1 class="text-center font-bold text-2xl mb-3">Form Edit Kajian</h1>
                    <form method="POST" action="{{route('majelis.update',$majeli->id)}}" class="max-w-lg mx-auto" x-data='eventsForm' x-ref='eForm'>
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div class="w-full px-1">
                                ...
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="BAM" type="text" name="name" id="name" value="{{$majeli->name}}" required />
                            </div>
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="institution">
                                    Kategori
                                </label>
                                <select class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" name="category" required>
                                    <option value="" disabled>-- Silahkan Pilih Kategori --</option>
                                    <option value="Internal" {{ $majeli->category == 'Internal' ? 'selected' : '' }}>Internal</option>
                                    <option value="Eksternal" {{ $majeli->category == 'Eksternal' ? 'selected' : '' }}>Eksternal</option>
                                </select>
                            </div>
                            <div class="w-full px-1">
                            ...
                            <div class="w-full px-1">
                                <div x-data="{ 
                                    ...
                                    <div x-show="open" @click.away="open = false" class="absolute bg-white border border-gray-300 rounded-md shadow-md mt-1 z-50 p-3">
                                        <input type="datetime-local" @change="selectDate" class="block w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 sm:text-sm" value="{{$majeli->start_date}}" />
                                    </div>
                                </div>
                            </div>
                            <div class="w-full px-1">
                                <div x-data="{ 
                                    ...
                                    <div x-show="open" @click.away="open = false" class="absolute bg-white border border-gray-300 rounded-md shadow-md mt-1 z-50 p-3" x-cloak>
                                        <input type="datetime-local" @change="selectDate" class="block w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 sm:text-sm" value="{{$majeli->end_date}}" />
                                    </div>
                                </div>
                            ...
                            <div class="w-full px-1 col-span-2">
                                ...
                                <textarea class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Apa Ya?" type="number" name="desc" id="desc"> {{$majeli->desc}} </textarea>
                            </div>
                        </div>
                        <div class="px-1">
